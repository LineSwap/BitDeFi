...
const supportedChainId = [ChainId.MANTLE]
...
export function createFarmFetcher(multicallv2: MultiCallV2) {
  const fetchFarms = async (
    ...
    const masterChefAddress = isTestnet ? masterChefAddresses[ChainId.BSC] : masterChefAddresses[ChainId.MANTLE]
  ...
  return {
    ...
    supportedChainId,
    isTestnet: (chainId: number) => ![ChainId.MANTLE, ChainId.ETHEREUM].includes(chainId),
  }
}
