...
export const multicallAddresses = {
  ...
  5001: '0xa39BE21a15aeB356b17EE75F0A3e94518236BfA0',
}
...
export function createMulticall<TProvider extends Provider>(
  ...
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.MANTLE) => {
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.MANTLE, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
